...
## מה נלמד מסבב העיצוב (09/08/2026, v5) — כללים שחלים על כל הכלים
- **תקציב אקסנט.** כשהאקסנט צובע כל מספר בעמוד — אף מספר לא בולט. הכלל שנקבע:
  האקסנט שייך ל*תשובה* (מספר הגיבור, הטבעת, ספירת הימים בשורת אירוע, סמן "אתה כאן",
  נקודות הרשת). מספרי סטטיסטיקה = `--ink`. זה מה שיצר היררכיה אמיתית בלי לגעת בפריסה.
- **הילה רדיאלית מאחורי טבעת התקדמות = כתם.** `radial-gradient` על העוטף זוהר גם סביב
  46% מהמעגל *שעדיין לא צויר*, ובכהה זה נראה ככתם חום. האור שייך לקו עצמו:
  `filter:drop-shadow(0 0 7px var(--glow))` על `.fill`. `--glow` הוא **טוקן נגזר-אקסנט**
  (`color-mix(accent 22-26%)`) — הילה עם צבע קשיח הייתה נשברת ב-5 הסוואצ'ים.
- **`scroll-reveal` על תוכן = דף ריק בכלים.** פאנלים ב-`opacity:0` שמחכים ל-IntersectionObserver
  נראים ריקים לכל מי שמרנדר בלי לגלול — כולל `verify-all.mjs` (הוא צילם עמוד שחור), הדפסה,
  ושחזור עמוד. **אל תתנה תוכן ב-callback** בכלי שמבטיח לעבוד אופליין. אם בכל זאת — רק
  לאלמנטים דקורטיביים, לעולם לא לגוף התוכן.
- **`aria-label` *מחליף* את התוכן.** ל-`#heroNum` היה `aria-label:'החלף יחידת ספירה'`,
  כלומר המספר היחיד שהמסך קיים בשבילו לא היה קריא בקורא-מסך, וכל `aria-live` הכריז
  את ההוראה במקום את הערך החדש. **כלל:** אלמנט שהוא גם תוכן וגם פקד — הערך פותח את
  ה-label, ההוראה באה אחריו.
- **הוראה כתובה = פקד חסר.** "לחיצה על המספר מחליפה יחידה" הוחלף בבורר מקטעים
  (`.seg`) שמראה את שלושת המצבים. תופס אותו מקום, לא צריך להסביר. **הדפוס:** כל
  מקטע נושא `border:1px solid transparent` ורק הפעיל צובע אותו — הוספת border רק
  בבחירה מזיזה את השכנים בפיקסל בכל החלפה.
- **שורה = אובייקט אחד.** שורת אירוע הייתה כרטיס + שני כפתורי אייקון מרחפים לידו =
  שלושה מתארים לאירוע אחד. הכרטיס (`background/border/shadow/overflow:hidden`) עבר
  ל-`.ev-row`, הכפתור הפנימי `.ev` נעשה שקוף, והפעולות הן עמודה מאחורי `border-inline-start`
  יחיד. מצב "בקרוב" = פס אקסנט `::before` ברוחב 3px, לא border בגוון שמטשטש את הצורה.
- **ריתמוס מקטעים:** `.panel{padding:2rem 1.25rem}` + `.panel+.panel{border-block-start}` +
  **`.panel:empty{display:none}`** (בלי זה, משתמש בלי פרופיל מקבל שלושה מקטעים ריקים
  עם padding וקו מפריד).
- **מערכת רדיוסים אחת:** פקדים `--r-sm:12px` (כפתורים, אינפוטים, `iconbtn`, `chip-ico`,
  `sheet-x`), משטחים `--r:16px` (כרטיסים, טוסט, תאי live), וכל מה שגלולה `999px`.
- **מספר גדול + מילת-יחידה קטנה מתחתיו** קריא יותר ממשפט מודגש ("נשארו 12 ימים").
  ב-`<360px` הבלוק נשכב לרוחב (`flex-direction:row`) במקום להישאר עמודה.

## היסטוריית שינויים גדולים
- **09/08/2026 (v5):** סבב עיצוב על `service-countdown` במסכה של מומחה UI — היררכיה,
  תקציב אקסנט, ריתמוס מקטעים, בורר יחידות גלוי, שורת אירוע כאובייקט אחד, הילה על
  קו הטבעת. פרטים מלאים בסעיף "מה נלמד מסבב העיצוב" למעלה. `sc-v6`, 76+16 בדיקות.
